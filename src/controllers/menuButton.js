...
import { checkTheStatusOfTheSelectedCells, frozenFirstRow, frozenFirstColumn, getRange } from "../global/api";
...
const menuButton = {
    ...
    initialMenuButton: function() {
        ...
        $("#luckysheet-settingExcelRange").click(function() {
            // *如果禁止前台编辑，则中止下一步操作
            if (!checkIsAllowEdit()) {
                tooltip.info("", locale().pivotTable.errorNotAllowEdit);
                return;
            ...
            // 检测是表格设置区域
            const checkISExcelAreaBorder = function(x) {
                return x.borderType === 'border-outside' && x.color === '#FF715E' && x.rangeType === "range" && x.selfType === "excel-area" && x.style === "4"
            }

            // 检测和已设置的区域是同一个区域
            const checkIsSameAreaBorder = function(x, range) {
                if(checkISExcelAreaBorder(x)) {
                    const oldRange = x.range[0];
                    const col_first = oldRange?.column[0];
                    const col_last = oldRange?.column[1];
                    const row_first = oldRange?.row[0];
                    const row_last = oldRange?.row[1];

                    const selectRange = range;
                    const selectCol_first = selectRange?.column[0];
                    const selectCol_last = selectRange?.column[1];
                    const selectRow_first = selectRange?.row[0];
                    const selectRow_last = selectRange?.row[1];
                    if(col_first === selectCol_first && col_last === selectCol_last && row_first === selectRow_first && row_last === selectRow_last) {
                        return true;
                    } else {
                        return false;
                    }
            ...
            if(Store?.config?.["borderInfo"]) {
                const originData = JSON.parse(JSON.stringify(Store))

                // console.log('config?.["borderInfo"]', originData?.config?.["borderInfo"]);

                for (let index = 0; index < range.length; index++) {
                    const rangeItem = range[index];
                    for (let i = 0; i < originData?.config?.["borderInfo"].length; i++) {
                        const item = originData.config["borderInfo"][i];
                        if(checkIsSameAreaBorder(item, rangeItem)) {
                            findIndexArr[index] = i;
                            console.log('相同', i);
                            cfg['borderInfo'][i] = null
                        } 
                        else {
                            if(checkISExcelAreaBorder(item)) {
                                if(cfg['borderInfo'][i]) {
                                    console.log('界内', i);
                                    findIndexArr[index] = i;
                                    cfg['borderInfo'][i].range = [rangeItem]
                                }
                            }
                        }
                    }
    
                    if(findIndexArr?.[index] === undefined) {
                        findIndexArr[index] = 0;
                        cfg["borderInfo"].push({
                            rangeType: "range",
                            borderType: type,
                            color: color,
            ...
            cfg["borderInfo"] = cfg["borderInfo"].filter(Boolean)


            console.log('cfg["borderInfo"]', cfg["borderInfo"]);
