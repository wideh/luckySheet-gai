...
export async function outputPDF({
  ...
  // 一页的高度， 转换宽度为一页元素的宽度
  var { width, height, data } = await toCanvas(element, contentWidth);
  ...
  if (footer) {
    // 页脚元素 经过转换后在PDF页面的高度
    var { height } = await toCanvas(footer, contentWidth);
    tfooterHeight = height;
  }

  if (header) {
    // 页眉元素 经过转换后在PDF的高度
    var { height } = await toCanvas(header, contentWidth);
    theaderHeight = height;
  }

  // 距离PDF左边的距离，/ 2 表示居中
  var baseX = (A4_WIDTH - contentWidth) / 2; // 预留空间给左边
  // 距离PDF 页眉和页脚的间距， 留白留空
  var baseY = 15;
  ...
  var rate = contentWidth / elementWidth;
  ...
  function traversingNodes(nodes) {
    for (var i = 0; i < nodes.length; ++i) {
      ...
      // 图片元素不需要继续深入，作为深度终点
      var isIMG = one.tagName === 'IMG';
      // table的每一行元素也是深度终点
      var isTableCol = one.classList && one.classList.contains('ant-table-row');
      // 特殊的富文本元素
      var isEditor = one.classList && one.classList.contains('editor');
  ...
  function traversingEditor(nodes) {
    ...
    for (var i = 0; i < nodes.length; ++i) {
      ...
      var { offsetHeight } = one;
      var offsetTop = getElementTop(one);
      var top = (contentWidth / elementWidth) * offsetTop;
  ...
  for (var i = 0; i < pages.length; ++i) {
    ...
    if (i < pages.length - 1) {
      // 获取当前页面需要的内容部分高度
      var imageHeight = pages[i + 1] - pages[i];
      // 对多余的内容部分进行遮白
      addBlank(
        0,
